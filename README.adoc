...
## 🔧 Atlan Fork - Version 3.0.1-atlan.1

This is a fork of the original Instaclustr Esop repository based on version 3.0.1, which provides support for Cassandra 3.x (we use Cassandra 3). While newer versions of Esop exist, this version is maintained for compatibility with our Cassandra 3 deployment.

**Added GCP Uniform Bucket-Level Access Support**
- New flag: `--gcp-uniform-bucket-level-access` (JSON property: `gcpUniformBucketLevelAccess`)
- When enabled, skips setting ACLs for GCP storage operations when bucket has uniform bucket-level access enabled
- Addresses https://github.com/instaclustr/esop/issues/89
- Defaults to `false` to maintain backward compatibility

This fork maintains full compatibility with the original esop functionality while adding support for modern GCP storage configurations.
